...
def polynomial_fit_features(interval_df, segment, degree=3):
    """Fit a polynomial and extract coefficients for position and orientation data."""
    ...
    poly_features = {}
    for col in pos_cols + ori_cols:
        times = np.arange(len(interval_df)) / 60  # assuming 60Hz sampling rate
        data = interval_df[col].values
        if len(data) >= degree + 1:  # Ensure we have enough points to fit the polynomial
            poly_fit = Polynomial.fit(times, data, degree)
            for i, coef in enumerate(poly_fit.convert().coef):  # Extract only coefficients
                poly_features[f'{col}_poly_coef_{i}'] = coef
        else:
            # If not enough data, fill with NaNs or zeros
            for i in range(degree + 1):
                poly_features[f'{col}_poly_coef_{i}'] = np.nan
...
def process_data(filepaths, time_window, data_type):
    ...
    for filepath in tqdm(filepaths, desc=f'Processing {data_type} data'):
        ...
        for interval, interval_df in df.groupby('time_interval'):
            ...
            # Add polynomial features for each segment
            segments = ['Head', 'LeftTouch', 'RightTouch']
            #for segment in segments:
            #    poly_features = polynomial_fit_features(interval_df, segment)
            #    stats = pd.concat([stats, pd.DataFrame([poly_features])], axis=1)
